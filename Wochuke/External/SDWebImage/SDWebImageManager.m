...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageManagerDelegate>)delegate options:(SDWebImageOptions)options
{
    ...
    {
        if (delegate && [delegate respondsToSelector:@selector(webImageManager:didFailWithError:)]) {
            [delegate webImageManager:self didFailWithError:nil];
        }
        return;
    }
...
- (void)imageDownloaderDidReceiveData:(SDWebImageDownloader *)downloader{
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if ([delegate respondsToSelector:@selector(webImageManager:didReceiveData:)])
            {
                [delegate performSelector:@selector(webImageManager:didReceiveData:) withObject:self withObject:downloader];
            }
