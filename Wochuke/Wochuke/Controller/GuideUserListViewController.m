...
@interface GuideUserListViewController ()<UITableViewDataSource,UITableViewDelegate,UserCellDeleagte>{
...
@implementation GuideUserListViewController
...
- (void)viewDidLoad
{
    ...
    [self reloadDatas];
    if (_actCode == 1) {
        _navBar.topItem.title = @"查看用户列表";
    }else if (_actCode == 2){
        _navBar.topItem.title = @"收藏用户列表";
    }
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        @try {
            ...
            @try {
                ...
                JCUserList * list = [proxy getUserListByGuide:userid guideId:_guide.id_ actCode:_actCode timestamp:nil pageIdx:pageIndex pageSize:pageSize];
                if (list) {
                    if (pageIndex == 0) {
...
-(void) followUser:(JCUser *)user;{
    if ([ShareVaule shareInstance].userId) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
            id<JCAppIntfPrx> proxy = nil;
            @try {
                proxy = [[ICETool shareInstance] createProxy];
                @try {
                    NSString *userid = [ShareVaule shareInstance].userId;
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [SVProgressHUD showSuccessWithStatus:@"服务访问异常"];
                });
            }
